...
class ReplayBuffer:
    def __init__(self, max_size, input_shape, n_actions, distill=False, n_extra_obs=0):
        self.mem_size = max_size
        self.mem_cntr = 0
        self.n_extra_obs = n_extra_obs
        self.distill = distill
    ...
    def store_transition(self, state, action, reward, state_, done, teacher_action=None):
        index = self.mem_cntr % self.mem_size
        self.state_memory[index] = state
        self.action_memory[index] = action
        if self.distill:
